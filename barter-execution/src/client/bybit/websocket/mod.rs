...
use barter_instrument::asset::{BaseAsset, QuoteAsset};
...
use rust_decimal::prelude::{FromPrimitive, Signed, Zero};
...
) -> Result<Option<Vec<UnindexedAccountEvent>>, ()> {
    let event = match payload.topic {
        BybitPayloadTopic::Order => {
            let orders: Vec<OrderUpdateData> = serde_json::from_str(payload.data.get()).unwrap();
            ...
            orders
                ...
                .map(|o| {
                    instruments
                        .contains(&o.symbol)
                        .then(|| to_unified_order(o, payload.timestamp))
                })
                .collect()
        }
        BybitPayloadTopic::Execution => {
            ...
            let executions: Vec<OrderExecutionData> =
            ...
            executions
                ...
                .map(|e| {
                    instruments
                        .contains(&e.symbol)
                        .then(|| to_unified_execution(e, payload.timestamp))
                        .flatten()
                })
                .collect()
        }
...
pub fn to_unified_execution(
    ...
    Some(AccountEvent {
        ...
        kind: AccountEventKind::Trade(Trade {
            ...
            fees: AssetFees {
                asset: QuoteAsset,
                fees: Decimal::from_f64(execution.exec_fee)?,
            },
        }),
    })
}
